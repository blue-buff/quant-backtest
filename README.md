...
| 一键流水线 | `qbt` CLI | 数据 → 训练 → 计划 → 真实规则回测 → 报告（详见 [qbt/README.md](qbt/README.md)） |

## 回测分层方法论

专业量化的标准做法是**分层解耦**，本项目按同构思路组织：

```
① 信号层（研究）   qlib 模型 → 预测分数（IC/ICIR 检验）
② 组合层（构建）   分数 → 月度 topK 等权目标（当前为最朴素方案，可升级为优化器）
③ 执行层（交易）   rqalpha 真实规则：T+1 / 涨跌停 / 印花税 / 100股整数倍
```

**为什么信号要跑真实规则验证**：宽松规则回测会放大假信号。本项目的实证——中证500 在简化规则下 IC 正常（0.013），但真实规则回测跑输指数 30.5 个百分点。真实规则这层存在的意义不是让模型更准，而是**拦住假信号**。成本模型与执行细节是两层之间最大的差距来源（当前为静态费率，未含冲击成本/滑点/停牌锁定，见风险声明）。
...
完整命令参考、配置说明与常见问题见 [qbt/README.md](qbt/README.md)。
