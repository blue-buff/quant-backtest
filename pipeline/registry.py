...
_flock_state = _threading.local()

@contextmanager
def _locked():
    """Cross-process lock around ledger writes (P7 T7: experiment get-or-create
    AND run writes share one lock, so concurrent import processes cannot race).
    Reentrant within a process (flock on a second fd would self-deadlock)."""
    if fcntl is None:
        yield
        return
    if getattr(_flock_state, "held", False):
        yield
        return
    LOCK_FILE.parent.mkdir(parents=True, exist_ok=True)
    f = open(LOCK_FILE, "w")
    fcntl.flock(f, fcntl.LOCK_EX)
    _flock_state.held = True
    try:
        yield
    finally:
        _flock_state.held = False
...
def log_run(exp_name, params=None, metrics=None, tags=None, artifacts=None):
    ...
    The whole write (experiment get-or-create + run write) holds the cross-process
    registry lock (P7 T7)."""
    with _locked():
        return _log_run_unlocked(exp_name, params, metrics, tags, artifacts)


def _log_run_unlocked(exp_name, params=None, metrics=None, tags=None, artifacts=None):
